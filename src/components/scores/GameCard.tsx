...
function TeamColumn({
  ...
  return (
    <div className={`flex flex-col items-center gap-1.5 w-[96px] shrink-0 transition-opacity duration-300 ${dimmed ? 'opacity-35' : ''}`}>
      ...
      <span className="text-[12px] font-bold text-[var(--c-text-90)] text-center leading-tight line-clamp-2 mt-0.5">
        {team.location}
      </span>
      {team.record ? (
        <span className="text-[10px] text-[var(--c-text-30)] font-medium">{team.record}</span>
      ) : null}
    </div>
  );
}
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-2 flex-1 px-2 min-w-0">
        <div className="flex items-center gap-3">
          <span className={`text-[42px] font-black tabular-nums leading-none tracking-tight ${awayWin ? 'text-[var(--c-text)]' : 'text-[var(--c-text-25)]'}`}>
            {game.away.score}
          </span>
          <span className="text-[var(--c-text-15)] text-2xl font-thin select-none">|</span>
          <span className={`text-[42px] font-black tabular-nums leading-none tracking-tight ${homeWin ? 'text-[var(--c-text)]' : 'text-[var(--c-text-25)]'}`}>
            {game.home.score}
          </span>
        </div>
        {isFinal && (
          <span className="text-[10px] font-black text-[var(--c-text-30)] uppercase tracking-[0.2em]">Final</span>
        )}
        {isLive && (
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 flex-1 px-2 min-w-0">
      <div className="flex flex-col items-center gap-1">
        <span className="text-[11px] font-semibold text-[var(--c-text-25)] uppercase tracking-widest">vs</span>
        <span className="text-[15px] font-bold text-[var(--c-text-80)] text-center leading-tight">
          {game.status.shortDetail}
        </span>
      </div>
      {game.broadcasts.length > 0 && (
        <span className="text-[10px] text-[var(--c-text-25)] text-center truncate max-w-full bg-[var(--c-surface-subtle)] px-2 py-0.5 rounded-full">
          {game.broadcasts[0]}
        </span>
      )}
    </div>
  ...
  return (
    <div
      className={`rounded-lg overflow-hidden transition-all duration-200 card-hover ${
        isLive
          ? 'live-card-glow bg-[var(--c-surface)]'
          : isFavGame
          ? 'bg-[var(--c-surface)] ring-1 ring-amber-400/20 shadow-[0_0_20px_rgba(251,191,36,0.05)]'
          : 'bg-[var(--c-surface)]'
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-[var(--c-overlay)] border-t border-[var(--c-border-faint)]">
        <div className="flex items-center gap-2 min-w-0">
          {isLive && game.situation?.pitcher && (
            <span className="text-[10px] text-[var(--c-text-30)] truncate max-w-[120px]">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          {hasLinescores && !isLive && (
            <button
              ...
              className="p-1 text-[var(--c-text-20)] hover:text-[var(--c-text-50)] transition-colors"
          ...
          <button
            ...
            className="p-1.5 text-[var(--c-text-20)] hover:text-royal-bright transition-colors rounded-lg hover:bg-royal/10"
      ...
      {expanded && hasLinescores && (
        <div className="px-3 pt-2 pb-3 bg-[var(--c-overlay)] border-t border-[var(--c-border-faint)]">
          <LinescoreTable
            away={game.away}
            home={game.home}
          ...
          {isLive && game.situation?.lastPlay && (
            <div className="mt-2 text-[11px] text-[var(--c-text-40)] italic leading-snug">
              {game.situation.lastPlay}
            </div>
          )}
          {isLive && game.situation?.batter && (
            <div className="text-[10px] text-[var(--c-text-30)] mt-1">
              AB: {game.situation.batter}
            </div>
          )}
          {!isLive && game.broadcasts.length > 0 && (
            <div className="mt-1.5 text-[10px] text-[var(--c-text-25)]">
              {game.broadcasts.join(' / ')}
            </div>
          )}
          {game.venue.name && (
            <div className="text-[10px] text-[var(--c-text-25)] mt-0.5">
