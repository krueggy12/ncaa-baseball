...
function Top25Row({ team, idx, maxScore }: RowProps) {
  const { isFavorite, toggleFavorite } = useFavorites();
  const { ref, revealClass, revealStyle } = useReveal(Math.min(idx * 35, 210));
  ...
  return (
    <div
      ref={ref}
      className={`flex items-center px-3 py-2.5 border-b border-[var(--c-border-faint)] last:border-b-0 ${revealClass} ${
        fav
          ? 'bg-amber-400/5'
          : idx % 2 === 0
          ? 'bg-[var(--c-surface)]'
    ...
    <div className="px-3 pb-6">
      ...
      <div className="rounded-md overflow-hidden border border-[var(--c-border)]">
        {rankings.map((team, idx) => (
          <Top25Row key={team.teamId} team={team} idx={idx} maxScore={maxScore} />
        ))}
      </div>
